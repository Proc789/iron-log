...
const generateShareText = (group, workouts) => {
  ...
  workouts.forEach((w) => {
    ...
    w.sets.forEach((s, i) => {
      if (s.weight || s.reps) {
        text += `  第${i + 1}組｜${s.weight ? s.weight + 'kg' : '-'} × ${
          s.reps ? s.reps + '次' : '-'
        }\n`;
        totalSets++;
      }
...
export default function App() {
  ...
  // History
  const [histGroup, setHistGroup] = useState('胸');
  ...
  useEffect(() => {
    setHistExercise(exercises[histGroup]?.[0] || '');
  }, [histGroup, exercises]);
  ...
  const loadUserData = async (uid, modules) => {
    ...
    if (!m) return;
    try {
      const exSnap = await m.getDoc(
        m.doc(db, 'users', uid, 'data', 'exercises')
      );
  ...
  const persistExercises = async (data) => {
    ...
    try {
      await fbModules.setDoc(
        fbModules.doc(db, 'users', user.uid, 'data', 'exercises'),
        { value: data }
      );
    } catch {}
  };
  ...
  const persistLogs = async (data) => {
    ...
    try {
      await fbModules.setDoc(
        fbModules.doc(db, 'users', user.uid, 'data', 'logs'),
        { value: data }
      );
    } catch {}
  };
  ...
  const signIn = async () => {
    if (!fbModules) return;
    try {
      await fbModules.signInWithPopup(fbModules.auth, googleProvider);
    } catch {}
  };
  ...
  const addWorkout = () => {
    ...
    setWorkouts((prev) => [
      ...prev,
      { exercise: ex, sets: [{ weight: '', reps: '' }] },
    ]);
  };
  const updateExercise = (wi, val) =>
    setWorkouts((prev) =>
      prev.map((w, i) => (i === wi ? { ...w, exercise: val } : w))
    );
  const addSet = (wi) =>
    setWorkouts((prev) =>
      prev.map((w, i) =>
        i === wi ? { ...w, sets: [...w.sets, { weight: '', reps: '' }] } : w
      )
    );
  const removeSet = (wi, si) =>
    setWorkouts((prev) =>
      prev
        .map((w, i) =>
          i !== wi ? w : { ...w, sets: w.sets.filter((_, j) => j !== si) }
        )
        .filter((w) => w.sets.length > 0)
    );
  const updateSet = (wi, si, field, val) =>
    setWorkouts((prev) =>
      prev.map((w, i) =>
        i !== wi
          ? w
  ...
  // History data
  const getMaxWeightTrend = () => {
    const result = [];
    Object.entries(logs)
      .sort()
      .forEach(([date, entries]) => {
        const relevant = entries.filter(
          (e) => e.group === histGroup && e.exercise === histExercise
        );
        ...
        if (max > 0) result.push({ date: formatDate(date), max });
      });
    return result;
  };
  ...
  const getWeeklyVolume = () => {
    ...
    Object.entries(logs).forEach(([date, entries]) => {
      ...
      const relevant = entries.filter((e) => e.group === histGroup);
      if (!weekMap[wk]) weekMap[wk] = 0;
      relevant.forEach((e) => {
        weekMap[wk] += e.sets.filter((s) => s.weight || s.reps).length;
      });
    });
    return Object.entries(weekMap)
      .sort()
      .slice(-8)
      .map(([wk, sets]) => ({ week: formatDate(wk), sets }));
  };
  ...
  const removeExercise = (g, ex) =>
    persistExercises({
      ...exercises,
      [g]: exercises[g].filter((e) => e !== ex),
    });
  ...
  const histAccent = GROUP_COLORS[histGroup];
          ...
          <>
            ...
            >
              {Object.keys(exercises).map((g) => (
                <button
                  ...
                  style={groupTabStyle(g, selectedGroup === g)}
                >
                  {GROUP_ICONS[g]} {g}
                </button>
              ))}
            ...
            {workouts.map((w, wi) => (
              <div key={wi} style={cardStyle}>
                <select
                  value={w.exercise}
                  onChange={(e) => updateExercise(wi, e.target.value)}
                  style={{
                    background: accentBg,
                ...
                {w.sets.map((s, si) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      value={s.weight}
                      onChange={(e) =>
                        updateSet(wi, si, 'weight', e.target.value)
                      }
                      style={inputStyle}
                    />
                    <span
                      style={{ color: '#ddd', fontSize: 14, flexShrink: 0 }}
                    >
                      ×
                    </span>
                    <input
                      ...
                      value={s.reps}
                      onChange={(e) =>
                        updateSet(wi, si, 'reps', e.target.value)
                      }
                      style={inputStyle}
                    />
                    <button
            ...
            {workouts.length === 0 && !showShare && (
              ...
              >
                ...
                <div
                  style={{ fontSize: 11, letterSpacing: '2px', color: '#ccc' }}
                >
                  點擊上方開始記錄
                </div>
              </div>
          ...
          <>
            ...
            >
              {Object.keys(exercises).map((g) => (
                <button
                  key={g}
                  onClick={() => setHistGroup(g)}
                  style={groupTabStyle(g, histGroup === g)}
                >
                  {g}
                </button>
              ))}
            ...
            <select
              value={histExercise}
              onChange={(e) => setHistExercise(e.target.value)}
              style={{
                background: '#fff',
                border: `1.5px solid ${histAccent}44`,
                borderRadius: 10,
                color: histAccent,
                fontSize: 14,
                fontWeight: 700,
            ...
            <div style={cardStyle}>
              ...
              {getMaxWeightTrend().length >= 1 ? (
                <ResponsiveContainer width="100%" height={150}>
                  <LineChart
                    data={getMaxWeightTrend()}
                  ...
                  >
                    ...
                    <Line
                      ...
                      stroke={histAccent}
                      strokeWidth={2.5}
                      dot={{ fill: histAccent, r: 4, strokeWidth: 0 }}
                      activeDot={{ r: 6 }}
                    />
                  </LineChart>
                </ResponsiveContainer>
            ...
            <div style={cardStyle}>
              ...
              {getWeeklyVolume().length >= 1 ? (
                <ResponsiveContainer width="100%" height={150}>
                  <BarChart
                    data={getWeeklyVolume()}
                    margin={{ top: 4, right: 4, left: -20, bottom: 0 }}
                  >
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis
            ...
            >
              {Object.keys(exercises).map((g) => (
                <button
                  ...
                  style={groupTabStyle(g, manageGroup === g)}
                >
                  {g}
                </button>
              ))}
              ...
              {exercises[manageGroup]?.map((ex, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '11px 0',
                    borderBottom:
                      i < exercises[manageGroup].length - 1
                        ? '1px solid #f5f5f5'
                        : 'none',
                  }}
                >
                  ...
                  <button
                    onClick={() => removeExercise(manageGroup, ex)}
                    style={{
                      background: 'transparent',
                      border: 'none',
                      color: '#ddd',
                    ...
                    onMouseOver={(e) =>
                      (e.currentTarget.style.color = '#E8413A')
                    }
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                <input
                  ...
                  onChange={(e) =>
                    setNewEx((prev) => ({
                      ...prev,
                      [manageGroup]: e.target.value,
                    }))
                  }
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') addExercise(manageGroup);
                  }}
                  style={{
                    flex: 1,
                    background: '#f8f8f8',
                ...
                />
                <button
                  onClick={() => addExercise(manageGroup)}
                  style={{
                    background: manAccent,
                    border: 'none',
                    borderRadius: 8,
                    color: '#fff',
                    fontSize: 13,
